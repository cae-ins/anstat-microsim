...
*               Seuils de pauvreté et indicateurs - Approche 2, Variante b    *
...
capture log close
log using "$prog\ehcvm_${pays}_pgm032b.log", replace text   

***********
** On utilise trois approches pour le seuil 2021, et pour chaque approche deux variantes
** Ce do-file traite de la deuxième approche et de sa seconde variante
** Approche 2: Construction du seuil de 2021 avec un nouveau panier
** Variante b: les déflateurs temporels sont ceux calculés du volet prix de l'EHCVM 
...
//
sort codpr
merge 1:1 codpr using "E:\vecteurVF_prix_kg.dta"
keep if _merge==3
drop _merge
//
...
save "$dataout_temp\seuil_nat.dta", replace   //* Za2021=201161 vs Za2018=186869; 7,6% semble un peu bien *//
...
compress
sort hhid 
save "$dataout\ehcvm_welfare_2b_${pays}.dta", replace
