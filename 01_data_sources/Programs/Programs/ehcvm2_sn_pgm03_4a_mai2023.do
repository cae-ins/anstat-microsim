...
*               Seuils de pauvreté et indicateurs - Approche 4, Variante a    *
...
capture log close
log using "$prog\ehcvm_${pays}_pgm04a.log", replace text   

***********
** On utilise quatre approches pour le seuil 2021, et pour chaque approche deux variantes
** Ce do-file traite de la quatrième approche et de sa première variante
** Approche 4: Seuil de 2021 avec panier de 2018, conso 2021, valorisé aux prix de 2021
** Variante a: les déflateurs temporels sont ceux du volet prix de l'EHCVM
** Les déflateurs spatiaux: rapport seuil national et seuils des zae  
***********
*
*
...
/* Exclure du panier les produits "autres", et aussi les repas hors ménages */
gen flag=(codpr==18 | codpr==19 | codpr==43 | codpr==59 | codpr==70 | codpr==75 | ///
...
*
** Comparaison avec le panier de 2018
*
rename (depant depan_tot) (depant21 depan_tot21)
merge 1:1 codpr using "$dataout_temp\ehcvm_panier2018_code2021.dta"
gen v3=_merge
drop _merge
tab v3, m
keep if v3==3
egen depant21t=sum(depant21)
egen depant18t=sum(depant_2018)
gen ind21=depant21t/depan_tot21 
dis ind21
gen ind18=depant18/depan_tot_2018 
dis ind18  /* produits communs font 83% conso 2021 et 81% conso 2018 */

keep codpr depant21 
sort codpr 
rename depant21 depant 
save "$dataout_temp\ehcvm_panier2018_code2021_conso2021.dta", replace
*
*
...
gen all=1
collapse (sum) popul38=hhsize [pw=hhweight], by(all)
compress
save "$dataout_temp\ehcvm_popul_dec37.dta", replace
*
*
...
use "$dataout_temp\ehcvm_panier2018_code2021_conso2021.dta", clear  /* Partir du panier de conso */

gen all=1
merge m:1 all using "$dataout_temp\ehcvm_popul_dec37.dta"
keep if _merge==3
drop _merge
...
preserve
  collapse (median) alpha0_min=alpha if (dtet>=dmin & dtet<=dmax) [pw=hhweight*dtet], by(all)
  save "$dataout_temp\zmin0.dta", replace  
restore

preserve 
  collapse (median) alpha0_max=alpha if (dalit>=dmin & dalit<=dmax) [pw=hhweight*dtet], by(all)
...
preserve
  collapse (median) alpha_min=alpha if dtet>=dmin & dtet<=dmax [pw=hhweight*dtet], by(zaemil)
  save "$dataout_temp\zmin1.dta", replace  
restore

preserve 
  collapse (median) alpha_max=alpha if dalit>=dmin & dalit<=dmax [pw=hhweight*dtet], by(zaemil)
...
*gen zref=zali0/alpha0_max
*gen zzae=zali/alpha_max
/*
...
/*  Comme en 2a, pb pour les seuils non-alimentaires et donc les déflateurs spatiaux
	2021	2018	Evolution
Za	212461.1	186869.0	1.136952089
Zna	144246.9	146572.0	0.984136561
Z	356708.0	333441.0	1.069778354
*/
...
compress
sort hhid 
save "$dataout\ehcvm_welfare_3a_${pays}.dta", replace

/********* Effacer les fichiers de travail */
erase "$dataout_temp\ehcvm_panier_seuil_nat.dta"
erase "$dataout_temp\ehcvm_popul_dec37.dta" 
